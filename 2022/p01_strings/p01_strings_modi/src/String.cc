...
void Strings::GetPrefix(std::vector<Strings>& prefix)
{
  ...
  aux_fix.Empty();
  // Iterando a través de los elementos de la cadena desde el primer elemento hasta el último.
...
void Strings::GetPrefixAndSufix(std::vector<Strings>& prefixandsufix)
{
  prefixandsufix.resize(0);  // Vaciar el vector.
  Strings aux;
  Symbol aux_fix;
  aux.alphabet_elements_ = alphabet_elements_;  // Asignación del alfabeto de la cuerda al alfabeto de la cuerda aux.
  prefixandsufix.push_back(aux);
  aux.elements_.resize(0);
  aux_fix.Empty();
  // Iterando a través de los elementos de la cadena desde el primer elemento hasta el último.
  for (int pos_1 = 0; pos_1 < elements_.size(); pos_1++)
